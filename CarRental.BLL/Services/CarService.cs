...
public class CarService : ICarService
{
    ...
    public async Task<IEnumerable<CarDto>> GetCarsByFilterAsync(CarFilterDto filter)
    {
        try 
        {
            Console.WriteLine($"🔍 CAR FILTER: Brand={filter.Brand}, Model={filter.Model}, MaxPrice={filter.MaxPrice}");
            
            var cars = await _carRepository.GetCarsByFilterAsync(filter.Brand, filter.Model, filter.MaxPrice);
        ...
        {
            Console.WriteLine($"💥 CAR SERVICE ERROR: {ex.Message}");
            throw;
        }
    }
